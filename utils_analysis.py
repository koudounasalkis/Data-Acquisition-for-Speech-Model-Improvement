...
def plotComparisonShapleyValues(
    ...
    if sort == "by_value":
        sh_score_1 = { 
            k: v for k, v in sorted(sh_score_1.items(), 
            key=lambda item: item[1])
            }
        sh_score_2 = { 
            k: v for k, v in sorted(sh_score_2.items(), 
            key=lambda item: item[1]) 
            }

    if sort == "alp":
        sh_score_1 = { 
            k: v*100 for k, v in sorted(sh_score_1.items(), 
            key=lambda item: item[0], reverse=True) 
            }
        sh_score_2 = { 
            k: v*100 for k, v in sorted(sh_score_2.items(), 
            key=lambda item: item[0], reverse=True) 
            }
    ...
    ax1.barh(
        ...
        sh_score_1.values(),
        align="center",
        color="C01" if not negative else "#83C4FA",
        height=h1,
        linewidth=linewidth,
        edgecolor="#0C4A5B",
    )
...
def plotShapleyValue(
        ...
        ax.barh(
            ...
            height=height,
            align="center",
            # color="#31DA40" if not negative else "#EF3B32",
            color="C01" if not negative else "#83C4FA",
            linewidth=linewidth,
            edgecolor="#0C4A5B",
        )
...
def plotMultipleSV_4(
        shapley_values_1=None,
        shapley_values_2=None,
        shapley_values_3=None,
        shapley_values_4=None,
        sortedF=True,
        height=0.1,
        linewidth=0.8,
        sizeFig=(6,4),
        ...
        sh_plt_1 = {str(",".join(list(k))): v for k, v in shapley_values_1.items()}
        sh_plt_2 = {str(",".join(list(k))): v for k, v in shapley_values_2.items()}
        sh_plt_3 = {str(",".join(list(k))): v for k, v in shapley_values_3.items()}
        sh_plt_4 = {str(",".join(list(k))): v for k, v in shapley_values_4.items()}
        ...
        ax.barh(
            np.arange(0.22,len(sh_plt_1)+0.22,1),
            doit(shapley_values_2, shapley_values_1),
            height=height,
            align="center",
            color="#83C4FA",
        ...
        ax.barh(
            np.arange(0.44,len(sh_plt_1)+0.44,1),
            doit(shapley_values_3, shapley_values_1),
            height=height,
            align="center",
            color="#53E260",
            hatch="||",
        ...
        ax.barh(
            np.arange(0.66,len(sh_plt_1)+0.66,1),
            doit(shapley_values_4, shapley_values_1),
            height=height,
            align="center",
            color="#FF7B70",
            hatch="\\",
...
def plotShapleyValue_1(
        ...
        positive_shap = [v for k, v in sh_plt.items() if v > 0]
        negative_shap = [v for k, v in sh_plt.items() if v < 0]
